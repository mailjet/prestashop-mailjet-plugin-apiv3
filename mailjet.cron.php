...
if ($mailjet->triggers['active']) {
    ...
    if (!isset($context->shop->name)) {
        $context->shop->name = Configuration::get('PS_SHOP_NAME');
    }
    if (!isset($context->shop->domain)) {
        $context->shop->domain = Configuration::get('PS_SHOP_DOMAIN');
    }
    ...
    $trigger_max = 9;
    if (!Configuration::get('PS_LOYALTY_POINT_VALUE')) {
        $trigger_max = 8;
    }
