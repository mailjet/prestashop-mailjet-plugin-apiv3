...
function getTable($header, $data, $id)
{
    ...
    foreach ($data as $d) {
        ...
        $html .= '<td><a href="config.php?type=addto' . Tools::safeOutput($id) . '&id' . Tools::safeOutput($id) . '=' .
            Tools::safeOutput($d['id_' . $id . 'condition']) . '">Add</a></td>';
        $html .= '</tr>';
    }
    return $html;
}
...
function getAdd($input, $type, $hidden = false)
{
    ...
    if ($hidden) {
        foreach ($hidden as $key => $value) {
            $html .= '<input type="hidden" name="' . Tools::safeOutput($key) .
    ...
    foreach ($input as $i) {
        $html .= '<tr><td>' .
            Tools::safeOutput($i) . '</td><td><textarea name="' . Tools::safeOutput($i) . '" ></textarea></td></tr>';
    }
...
if (Tools::getValue('type') == 'baseadd') {
    ...
    echo getTable(
        array('id', 'idbase', 'label', 'jointable'),
        Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'sourcecondition'),
        'source'
    );
} else {
    echo getAdd(array('label', 'tablename'), 'baseadd');
    echo getTable(
        array('id', 'label', 'tablename'),
        Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'basecondition'),
        'base'
    );
}
