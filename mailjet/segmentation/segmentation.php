...
class Segmentation extends Module
{
	...
	public function install()
	{
		...
		AND $this->loadConfiguration() && parent::install() 
	...
	{		
		...
		$content = '
			<link rel="stylesheet" type="text/css" href="../modules/mailjet/'.$this->name.'/views/js/datepicker/datepicker.css" />
			<link rel="stylesheet" type="text/css" href="../modules/mailjet/'.$this->name.'/views/css/style.css" />
			<link rel="stylesheet" type="text/css" href="../modules/mailjet/'.$this->name.'/views/css/bundlejs_prestashop.css" />
			<script type="text/javascript" src="'._MODULE_DIR_.'mailjet/'.$this->name.'/views/js/datepicker.js"></script>
			<script type="text/javascript" src="'._MODULE_DIR_.'mailjet/'.$this->name.'/views/js/fonction.js"></script>
			<script type="text/javascript" src="'._MODULE_DIR_.'mailjet/'.$this->name.'/views/js/main.js"></script>
			<script type="text/javascript" src="'._MODULE_DIR_.'mailjet/'.$this->name.'/views/js/bundlejs_prestashop.js"></script>
			<script type="text/javascript">
				var tokenV = "'.$token = Tools::getValue('token').'";
				var ajaxFile =  "'._MODULE_DIR_.'mailjet/'.$this->name.'/views/templates/ajax/ajax.php";
				var ajaxSyncFile =  "'._MODULE_DIR_.'mailjet/'.$this->name.'/views/templates/ajax/sync.php";
				var ajaxBundle =  "'._MODULE_DIR_.'mailjet/'.$this->name.'/views/templates/ajax/bundlejs_prestashop.php";
				...
				<div class="newFilter custo">
					...
					<div class="div_new_filter">
						...
						<br />
							...
							<button id="save" class="my_button right"><img src="../modules/mailjet/'.$this->name.'/views/img/save.png" /> '.$this->l('Save').'</button>
							<button id="view" class="my_button right"><img src="../modules/mailjet/'.$this->name.'/views/img/table.png" /> '.$this->l('View').'</button>
							<button id="export" class="my_button right"><img src="../modules/mailjet/'.$this->name.'/views/img/page_excel.png" />'.$this->l('Export').'</button>
							<button id="sync" class="my_button right"><img src="../modules/mailjet/'.$this->name.'/views/img/sync.png" />'.$this->l('Create / Update Mailjet list').'</button>
							<div class="perc_sync">Synchronisation : <span id="perc_sync_value">0</span>%</div>
							'.$this->newLine(false).'
						</div>
						<div id="load" style="display:none;"><center><img src="../modules/mailjet/'.$this->name.'/views/img/load.gif" ></center></div>
						<div id="result"></div>
						'.$this->getBlockGroup().'
					</div>
				</div>
	...
	public function newLine($cond = true)
	{
		return '
			<table id="newLine" style="display:none;">
				<tr id="#####">
					<td id="action#####">
						<a href="javascript:addLine();" class="add"><img src="../modules/mailjet/'.$this->name.'/views/img/add.png" /></a>
						<a href="javascript:delLine(#####);" class="delete"><img src="../modules/mailjet/'.$this->name.'/views/img/delete.png" /></a>
					</td>
					<td id="id#####">#####</td>
		...
		{				
			...
			$html .= '
				<tr class="trSelect" id="list'.$r['id_filter'].'">
					...
					<td>'.$group_name.'</td>
					<td><a href="javascript:deleteFilter('.$r['id_filter'].');"><img src="../modules/mailjet/'.$this->name.'/views/img/delete.png" /></a></td>
				</tr>';
		}
	...
	public function l($string, $specific = false) // ** **
	{
		if ($string == parent::l($string, $specific))
		{
			...
			if (file_exists($trad_file))
			{
				...
				$key = '<{segmentmodule}prestashop>segmentmodule_'.md5(str_replace('\'', '\\\'', $string));
	...
	public function getQuery($post, $live, $limit = false, $speField = '')
	{
		...
		if ($live)
		{
			...
			for ($i = 0; $i < $nb; $i++)
			{
				...
				for ($j=$sub_base; $j<count($post['fieldSelect']); $j++)
				{
	...
	public function getLine($number, $rule_a, $rule_action, $idbase, $idsource, $idfield, $data, $value1, $value2)
	{
		return '<tr id="'.$number.'">
					<td id="action'.$number.'">
						<a class="add" href="javascript:addLine();"><img src="../modules/mailjet/'.$this->name.'/views/img/add.png" /></a>
						<a class="delete" href="javascript:delLine('.$number.');"><img src="../modules/mailjet/'.$this->name.'/views/img/delete.png" /></a>
					</td>
		...
		$content .=	'
			...
			<div class="rowAction" id="attrib">
				...
				<div class="size3">
					...
					<img src="../modules/mailjet/'.$this->name.'/views/img/table.png" /> '.$this->l('Assign now').'
					</button><img src="'.__PS_BASE_URI__.'modules/mailjet/'.$this->name.'/views/img/load.gif" id="wait" style="margin:3px;display:none;" />
