...
class Mailjet extends Module
{
	...
	public static function mj_mail_server() {
		$mj = new Mailjet(); return $mj->mj_mail_server;
	}
	public static function mj_mail_port() {
		$mj = new Mailjet(); return $mj->mj_mail_port;
	}
	...
	*/
	...
	public function postProcess()
	{
		...
		else if (Tools::isSubmit('MJ_set_connect'))
		{
			...
			/*$this->account['API_KEY'] = Tools::getValue("mj_api_key");
			 $this->account['SECRET_KEY'] = Tools::getValue("mj_secret_key");
			$this->account['EMAIL'] = Tools::getValue("mj_email");
			$this->account['PASSWD'] = Tools::getValue("mj_passwprd");
		...
		{
			$api = MailjetTemplate::getApi();
			if (Tools::getValue('MJ_account_tracking_clicks')=='1') $tracking_clicks = true; else $tracking_clicks = false;
			if (Tools::getValue('MJ_account_tracking_openers')=='1') $tracking_openers = true; else $tracking_openers = false;
	...
	public function displayAccount()
	{
		...
		// 			if ($test->type == 'email')
			// 				$is_senders = 1;
			// 			elseif ($test->type == 'domain')
			// 				$is_domains = 1;

			//$domain = $api->userDomainstatus();
			//$domains = $api->getTrustDomains();

			// test for the activation root file
			//$root_file = 0;
			//$available_domain = 0;
			// 		foreach ($domains->domains as $domain)
				// 		{
				// 			if ( ($domain->domain==Configuration::get('PS_SHOP_DOMAIN')) || ($domain->domain==Configuration::get('PS_SHOP_DOMAIN_SSL')) )
					// 			{
					// 				$available_domain = 1;
					// 				if (file_exists(dirname(__FILE__).'/../../'.$domain->file_name)) $root_file = 1;
					// 			}
...
public function checkPlanValidity()
{
	...
	if (Tools::getValue('MJ_request_page') != "PRICING" && ($plan->uname == "free" || $plan->uname == "bronze"))
	{
		...
		// TODO : ACTIVER à la mise en ligne !
...
{
	return (bool)($this->account['AUTHENTICATION'] && !(empty($this->account['API_KEY'])) && !empty($this->account['SECRET_KEY']));
}
