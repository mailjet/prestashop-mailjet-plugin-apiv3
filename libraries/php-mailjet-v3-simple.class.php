...
class MailjetPHPWrapper
{
    ...
    public function curl_setopt_custom_postfields($curl_handle, $postfields, $headers = null)
    {
        ...
        curl_setopt(
            $curl_handle, CURLOPT_HTTPHEADER, array(
    ...
    public function requestUrlBuilder($resource, $params = array(), $request, $id)
    {
        ...
        } elseif ($resource == "uploadCSVContactslistData") {
        ...
        } elseif (($resource == "addHTMLbody") || ($resource == "getHTMLbody")) {
        ...
        } elseif (in_array($resource, self::$_newsletterResources)) {
        ...
        } elseif (in_array($resource, self::$_templateResources)) {
            $this->call_url = $this->makeUrlFromFilter('REST', 'template', $params['ID'], $resource);
        } elseif (in_array($resource, self::$_dnsResources)) {
            $this->call_url = $this->makeUrlFromFilter('REST', 'dns', $params['ID'], $resource);
        } elseif (in_array($resource, self::$_contactResources)) {
        ...
        } elseif ($resource == "contactManageManyContacts") {
            $this->call_url = $this->apiUrl . "/REST/contact/managemanycontacts";
        } elseif (in_array($resource, self::$_contactslistResources)) {
    ...
    public function sendRequest($resource = false, $params = array(), $request = "GET", $id = '')
    {
        ...
        if (($request == 'POST') || ($request == 'PUT')) :
            ...
            $paramsFiltered = array_filter(
                array_keys($params), function ($k) {
