...
<script type="text/javascript">
    var MJ_page_name = "{$MJ_page_name|escape:'javascript':'UTF-8'|default:''}";
        ...
        <div id="MJ_tab_menu">
            <ul id="MJ_tab">
                ...
                {foreach from=$MJ_tab_page key=MJ_key item=MJ_title}
                    ...
                    {else}
                        <li {if $MJ_page_name == $MJ_key}class="active"{/if}>
                            <a href="{$MJ_adminmodules_link|escape:'htmlall':'UTF-8'|default:''}&{$MJ_REQUEST_PAGE_TYPE|escape:'htmlall':'UTF-8'|default:''}={$MJ_key|escape:'htmlall':'UTF-8'|default:''}">{$MJ_title|escape:'htmlall':'UTF-8'|default:''}</a>
                        </li>
                    {/if}
    ...
    <div class="alert error">
        ...
        <ul class="mj_errors">
            {foreach from=$MJ_errors item=current_error}
                <li>{$current_error|escape:'quotes':'UTF-8'|default:''}</li>
                {/foreach}
        </ul>
    </div>
{/if}
