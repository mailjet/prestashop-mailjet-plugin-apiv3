...
<div id="mj_campaigns_page" class="center_page" style="width:initial;max-width:960px;">
    <form id="create" method="post" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'|default:''}">
        <input type="hidden" name="MJ_submitCampaign2" value=1 />
        <fieldset>
            <legend>{l s='New Campaign' mod='mailjet'}</legend>
            <div class="button" style="padding:10px;"><b style="font-size:15px;">{l s='Campaign' mod='mailjet'} : &#171; {$campaign.title|escape:'htmlall':'UTF-8'|default:''} &#187;</b></div>
            <br />
                ...
                <div class="button" style="text-align:left;">
                    ...
                    {if $lang=="en"}
                        <select id="month" name="month" style="text-align:right;">
                            {for $var=1 to 12}<option value="{$var|escape:'htmlall':'UTF-8'}"{if $var==$month} selected{/if}>{$months.$var|escape:'htmlall':'UTF-8'}</option>{/for}
                        </select> 
                        <select id="day" name="day" style="text-align:center;">
                            {for $var=1 to 31}<option value="{$var|escape:'htmlall':'UTF-8'}"{if $var==$day} selected{/if}>{$var|escape:'htmlall':'UTF-8'}</option>{/for}
                        </select> 
                        <select id="year" name="year">
                            {for $var=0 to 9}<option value="{$var+$year|escape:'htmlall':'UTF-8'}">{$var+$year|escape:'htmlall':'UTF-8'}</option>{/for}
                        </select>
                    {else}
                    ...
                    <select id="hour" name="hour" style="text-align:right;">
                        {for $var=0 to 23}<option value="{$var|escape:'htmlall':'UTF-8'}"{if $var==$hour} selected{/if}>{$var|escape:'htmlall':'UTF-8'}</option>{/for}
                    </select>{l s='H' mod='mailjet'} 
                    <select id="minute" name="minute">
                        {for $var=0 to 59}<option value="{$var|escape:'htmlall':'UTF-8'}"{if $var==$min} selected{/if}>{$var|escape:'htmlall':'UTF-8'}</option>{/for}
    ...
    <script>
        ...
        {
            if (sendlater) {
                var today = create_date_object("{$today|escape:'htmlall':'UTF-8'|default:''}")
                // *****
        {literal}
