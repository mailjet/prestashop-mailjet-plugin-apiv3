...
{$MJ_templates.SEGMENTATION|escape:'htmlall':'UTF-8'|default:''}
        ...
        <div class="newFilter custo">
            ...
            <table class="table space" id="list" style="{if !$mj_filter_list}display:none;{/if}">
                ...
                {foreach from=$mj_filter_list item=filter}
                    <tr class="trSelect" id="list{$filter.id_filter|escape:'htmlall':'UTF-8'|default:'0'}">
                        <td>{$filter.id_filter|escape:'htmlall':'UTF-8'|default:'0'}</td>
                        <td><b>{$filter.name|escape:'htmlall':'UTF-8'|ucfirst}</b></td>
                        <td>{$filter.description|escape:'htmlall':'UTF-8'|default:''}</td>
                        <td>{if ($filter.assignment_auto)}{if ($filter.replace_customer)}{$mj_trads[97]|escape:'htmlall':'UTF-8'|default:'Replace'}{else}{$mj_trads[98]|escape:'htmlall':'UTF-8'|default:'Add'}{/if}{else}--{/if}</td>
                        <td>{if ($filter.assignment_auto)}{$mj_trads[96]|escape:'htmlall':'UTF-8'|default:'in real time'}{else}--{/if}</td>
                        <td>
                            {if $mj_groups}
                                {foreach from=$mj_groups item=group}
                                    {if ($filter.id_group == $group.id_group)}
                                        {$group.name|escape:'htmlall':'UTF-8'|default:'?'}
                                    {/if}
                            {/foreach}{/if}
                        </td>
                        <td><a href="javascript:deleteFilter({$filter.id_filter|escape:'htmlall':'UTF-8'|default:'0'});"><img src="../modules/mailjet/views/img/delete.png" /></a></td>
                    </tr>
                {/foreach}
            </table>
            <br />
            ...
            <div class="div_new_filter">
                ...
                <div class="nameFilter">
                    <form method="post" id="mainForm" action="../modules/mailjet/views/templates/admin/export.php">
                        ...
                        <br />
                        <input type="hidden" value="{$mj_id_employee|escape:'htmlall':'UTF-8'|default:'0'}" name="id_employee" />
                        <input type="hidden" value="{$mj_token|escape:'htmlall':'UTF-8'|default:'0'}" name="token" />
                            ...
                            <td>
                                <select id="baseSelect#####" name="baseSelect[]" class="baseSelect fixed">
                                    ...
                                    {foreach from=$mj_base_select item=base}
                                        <option value="{$base.id_basecondition|escape:'htmlall':'UTF-8'|default:'-1'}">{$mj_trads[$base.label]|escape:'htmlall':'UTF-8'|default:''}</option>
                                    {/foreach}
                                </select>
                            </td>
                ...
                <div class="blocAction">
                    ...
                    <fieldset class="custo">
                        ...
                        <div class="rowAction">
                            ...
                            <span class="displayInlineBlock width200 verticalAlignTop">
                                <select id="groupUser">
                                    ...
                                    {if $mj_groups}
                                        {foreach from=$mj_groups item=group}
                                            <option value="{$group.id_group|escape:'htmlall':'UTF-8'|default:'0'}">{$group.name|escape:'htmlall':'UTF-8'|default:'?'}</option>
                                        {/foreach}
                                    {/if}
                                </select>
                            </span>
                        ...
                        <div class="rowAction" id="type">
                            ...
                            <span class="help">{l s='Add: If the client belongs to the selected group without losing its other groups' mod='mailjet'}.<br/>
                        ...
                        <div class="rowAction" id="attrib">
                            ...
                            <span class="displayInlineBlock width200 verticalAlignTop">
                                ...
                                <img src="{$mj__PS_BASE_URI__|escape:'htmlall':'UTF-8'|default:'/'}modules/mailjet/views/img/load.gif" id="wait" style="display:none;" />
                            </span>
