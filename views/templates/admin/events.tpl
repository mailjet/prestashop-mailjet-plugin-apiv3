...
*}<div class="center_page">
    <div id="events">
                ...
                <div class="error">
                    ...
                    <ul>
                        {foreach from=$MJ_events_form_success item=id_mj_events}
                            <li>{$id_mj_events|escape:'htmlall':'UTF-8'|default:''}</li>
                            {/foreach}
                    </ul>
                </div>
            {/if}
        ...
        {if !count($MJ_events_list)}
            ...
            <div class="warn">
                &nbsp; {l s='To activate Events, yous must go to your Mailjet account : ' mod='mailjet'} <a href="https://app.mailjet.com/account/triggers" target="_blank"><u>https://app.mailjet.com/account/triggers</u></a><br />
                <br />
                <b >
                    {l s='Specify the' mod='mailjet'} <span>Endpoint Url</span> : <input type="text" value="{$url|escape:'htmlall':'UTF-8'}"/><br />
            ...
            <form id="eventsForm" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'|default:''}" method="POST">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="title">ID</th>
                                    ...
                                    <th class="title">{$title|escape:'htmlall':'UTF-8'|default:''}</th>
                                    {/foreach}
                            </tr>
                        </thead>
                        <tbody>
                ...
                <div class="MJ_event_link">
                    ...
                    {if $MJ_paging.prev}
                        ...
                        <a href="{$MJ_adminmodules_link|escape:'htmlall':'UTF-8'|default:''}&{$MJ_REQUEST_PAGE_TYPE|escape:'htmlall':'UTF-8'|default:''}={$MJ_page_name|escape|default:''}&page={$calc|escape:'htmlall':'UTF-8'|default:''}"><</a>
                    {/if}
                    <a href="javascript:void(0)">{$MJ_paging.current_page|escape:'htmlall':'UTF-8'|default:''}</a>
                    {if $MJ_paging.next}
                        {assign var=calc value=$MJ_paging.current_page + 1}
                        <a href="{$MJ_adminmodules_link|escape:'htmlall':'UTF-8'|default:''}&{$MJ_REQUEST_PAGE_TYPE|escape:'htmlall':'UTF-8'|default:''}={$MJ_page_name|escape:'htmlall':'UTF-8'|default:''}&page={$calc|escape:'htmlall':'UTF-8'|default:''}">></a>
                        <a href="{$MJ_adminmodules_link|escape:'htmlall':'UTF-8'|default:''}&{$MJ_REQUEST_PAGE_TYPE|escape:'htmlall':'UTF-8'|default:''}={$MJ_page_name|escape:'htmlall':'UTF-8'|default:''}&page={$MJ_paging.last|escape:'htmlall':'UTF-8'}">>></a>
                    {/if}
                </div>
            </form>
